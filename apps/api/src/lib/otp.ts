...
const { generateSecret, generateURI, verifySync } = _require('otplib') as {
  ...
  generateURI: (opts: { strategy: string; label: string; issuer: string; secret: string }) => string
  verifySync: (opts: { token: string; secret: string; strategy: string }) => { valid: boolean }
}
