...
export async function setupTotpAction(): Promise<{ error?: string; secret?: string; qrCode?: string; uri?: string }> {
  ...
  try {
    ...
    return { error: e instanceof ApiError ? e.message : 'Failed to set up 2FA' }
  }
}

export async function enableTotpAction(code: string): Promise<{ error?: string }> {
  const token = await getAccessToken()
  if (!token) redirect('/login')
  try {
    ...
    return { error: e instanceof ApiError ? e.message : 'Failed to enable 2FA' }
  }
}

export async function disableTotpAction(code: string): Promise<{ error?: string }> {
  const token = await getAccessToken()
  if (!token) redirect('/login')
  try {
    ...
    return { error: e instanceof ApiError ? e.message : 'Failed to disable 2FA' }
  }
}
...
export async function resendVerificationAction(): Promise<{ error?: string }> {
  ...
  try {
    ...
    return { error: e instanceof ApiError ? e.message : 'Failed to resend verification email' }
  }
}
