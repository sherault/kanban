        ...
        <form action={formAction} className="p-6 space-y-4">
          ...
          {state.result ? (
            <div className="bg-green-50 border border-green-200 text-green-700 text-sm rounded-md px-3 py-2 space-y-1">
              ...
              <p>
                {state.result.imported} task
                {state.result.imported !== 1 ? "s" : ""} imported
              </p>
              {state.result.skipped > 0 && (
                <p className="text-green-600">
                  {state.result.skipped} row
                  {state.result.skipped !== 1 ? "s" : ""} skipped (missing
                  required fields)
                </p>
              )}
            </div>
            ...
            <>
              ...
              <div className="text-xs text-gray-500 space-y-1">
                ...
                <p className="font-mono bg-gray-50 px-2 py-1 rounded text-xs">
                  title, description, objective, start_date, end_date, tags,
                  global_subject, background_color, column
                </p>
                <p>
                  Only <code>title</code>, <code>startDate</code>, and{" "}
                  <code>endDate</code> are required.
                </p>
              </div>
            </>
          )}
