...
export function TaskDetailSidebar({
  ...
  const [task, setTask] = useState<TaskDto>(initialTask as TaskDto);
  ...
  useEffect(() => {
    ...
    if (isShell) {
      // If it's a shell, only fetch if we don't have it or if it's a different task
      if (!task || task.id !== initialId) {
  ...
  useEffect(() => {
    const ids = task.linkedTaskIds || [];
    if (task.id && ids.length > 0) {
