...
import { updateMemberRoleAction, removeMemberAction, transferOwnershipAction, deleteOrgAction } from '@/actions/orgs'
...
export function MembersSection({ members, orgId, currentUserId }: Props) {
  ...
  const otherMembers = members.filter((m) => m.userId !== currentUserId && m.role !== 'owner')
  ...
  function transferOwnership(toUserId: string) {
    ...
    startTransition(async () => {
      ...
      if (result.error) { setError(result.error); setTransferTarget(null) }
      else router.refresh()
    })
                ...
                <div className="flex items-center gap-2 shrink-0">
                  {canManage && !memberIsOwner && !isSelf ? (
                    <select
                      value={m.role}
                      disabled={isPending}
                      onChange={(e) => changeRole(m.userId, e.target.value as 'member' | 'manager')}
        ...
        return (
          <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-xl shadow-xl w-full max-w-sm p-6 space-y-4">
              <h3 className="text-base font-semibold text-gray-900">Transfer ownership</h3>
              <p className="text-sm text-gray-600">
                Transfer ownership to <strong>{target?.user.displayName}</strong>? You will become a manager and lose owner privileges.
              </p>
          ...
          <div className="border border-red-200 rounded-lg p-4 space-y-3">
            <div>
              ...
              <p className="text-xs text-gray-500 mt-0.5">
                Permanently deletes the organization, all its projects, and tasks. This cannot be undone.
                {otherMembers.length > 0 && ' Transfer ownership first if you want another member to keep it.'}
              </p>
            </div>
