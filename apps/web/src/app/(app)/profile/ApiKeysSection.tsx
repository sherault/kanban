...
export function ApiKeysSection({ initialKeys }: Props) {
  ...
  if (state.created && (!newKey || newKey.id !== state.created.id)) {
    ...
    setKeys((prev) => [state.created!, ...prev.filter((k) => k.id !== state.created!.id)])
  }
    ...
    <div className="space-y-6">
      ...
      <form action={formAction} className="flex gap-3 items-end">
        <div className="flex-1">
          <label htmlFor="label" className="block text-sm font-medium text-gray-700 mb-1">Key label</label>
          <input
      ...
      {newKey && (
        <div className="bg-amber-50 border border-amber-200 rounded-md p-4 space-y-2">
          <p className="text-sm font-medium text-amber-800">Copy your new API key — it won't be shown again:</p>
          <code className="block text-xs font-mono bg-white border border-amber-200 rounded px-3 py-2 break-all select-all">
            {newKey.rawKey}
          </code>
          <button onClick={() => setNewKey(null)} className="text-xs text-amber-700 hover:text-amber-900">Dismiss</button>
        </div>
      )}

      {revokeError && (
        <div className="bg-red-50 border border-red-200 text-red-700 text-sm rounded-md px-3 py-2">{revokeError}</div>
      )}
            ...
            <li key={key.id} className="py-3 flex items-center justify-between gap-4">
              <div>
                ...
                <p className="text-xs text-gray-400">
                  Created {new Date(key.createdAt).toLocaleDateString('en', { month: 'short', day: 'numeric', year: 'numeric' })}
                  {key.lastUsedAt && ` · Last used ${new Date(key.lastUsedAt).toLocaleDateString('en', { month: 'short', day: 'numeric' })}`}
                </p>
              </div>
              <button
