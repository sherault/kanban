...
import { setupTotpAction, enableTotpAction, disableTotpAction } from '@/actions/profile'
  ...
  return (
    <section>
      ...
      <p className="text-sm text-gray-500 mb-4">
        Use an authenticator app (Google Authenticator, Authy, 1Password…) to add a second layer of security.
      </p>

      {error && (
        <div className="mb-4 bg-red-50 border border-red-200 text-red-700 text-sm rounded-md px-3 py-2">{error}</div>
      )}
      {success && (
        <div className="mb-4 bg-green-50 border border-green-200 text-green-700 text-sm rounded-md px-3 py-2">{success}</div>
      )}

      {step === 'idle' && (
        <div className="flex items-center gap-4">
          <span className={`text-sm font-medium ${totpEnabled ? 'text-green-600' : 'text-gray-500'}`}>
