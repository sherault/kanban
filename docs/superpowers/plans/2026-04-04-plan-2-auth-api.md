...
const OPTIONS =
  process.env["NODE_ENV"] === "test" ? { timeCost: 1, memoryCost: 1024 } : {};
...
function makeApp(
  ...
  app.get(
    "/:orgId/resource",
    authz.requireOrgRole(minRole, (c) => c.req.param("orgId")),
    (c) => c.json({ ok: true }),
  );
  return app;
}
...
describe("DELETE /organizations/:orgId", () => {
  it("returns 422 when deleting the only org", async () => {
    ...
    const createRes = await app.request("/organizations", {
      ...
      body: JSON.stringify({ name: "Solo Org" }),
    });
    const { id: orgId } = (await createRes.json()) as { id: string };
  ...
  it("deletes the org when owner has another org", async () => {
    const { app, accessToken, close } = await setup();
    const r1 = await app.request("/organizations", {
      method: "POST",
      headers: {
        ...authHeaders(accessToken),
        "Content-Type": "application/json",
    ...
    const r2 = await app.request("/organizations", {
      method: "POST",
      headers: {
        ...authHeaders(accessToken),
        "Content-Type": "application/json",
...
router.post(
  ...
  authz.requireOrgRole("manager", (c) => c.req.param("orgId")),
  (c) => {
    const result = invSvc.createInvitation(
      c.req.param("orgId"),
      c.get("userId"),
    );
...
describe("DELETE /profile/api-keys/:keyId", () => {
  ...
  it("returns 404 for a key that belongs to another user", async () => {
    ...
    const idSvc = new IdentityService(
      (app as unknown as { db: ReturnType<typeof createTestDb>["db"] }).db,
    );
