...
export default function NewOrgPage() {
  ...
  return (
    <div className="max-w-md">
      ...
      <h1 className="text-2xl font-bold text-gray-900 mb-6">New organization</h1>
  ...
  return (
    <div>
      <nav className="flex items-center gap-1 text-sm text-gray-500 mb-6">
        ...
        <Link href={`/orgs/${params.orgId}`} className="hover:text-gray-900 transition-colors">
          Projects
        </Link>
      </nav>
      {children}
...
export default function ProjectBoardPage({
  ...
  return (
    <div className="text-center py-24">
      ...
      <Link href={`/orgs/${params.orgId}`} className="text-sm text-blue-600 hover:underline">
        ← Back to projects
      </Link>
    </div>
  )
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-8">
          <div className="mb-6">
            ...
            <p className="text-sm text-gray-500 mt-1">
              Join <span className="font-medium text-gray-700">{orgName}</span> on Kanban
            </p>
          </div>
