...
- `get_task_history`: Get the change history of a task, newest first — who changed which field, from what to what, and when. Supports a `field` filter and pagination; long values are truncated to 500 characters. `field: "note"` yields the task log book — one entry per `append_task_note` call, holding only the appended text.
